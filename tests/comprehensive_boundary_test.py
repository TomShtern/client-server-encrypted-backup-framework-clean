...
def test_file_transfer(file_path, expected_size, username):
...
def main():
    ...
    for i, size in enumerate(test_sizes, 1):
        try:
            ...
            # Create test file
            test_file = create_test_file(size, f"{size//1024}KB_{size%1024}B")
    ...
    if not failed_sizes:
        ...
        if successful_sizes:
            ...
            min_failure = min(failed_sizes) if failed_sizes else float('inf')
        ...
        for size in failed_sizes:
            error = results[size]['error'] or 'Unknown'
        ...
        for error, sizes in error_groups.items():
            size_ranges = [f"{s}B" for s in sizes[:3]]  # Show first 3
            if len(sizes) > 3:
