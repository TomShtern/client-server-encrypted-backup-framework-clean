...
# ---- Settings field builders registry ----
FIELD_BUILDERS: Dict[str, Callable[[Dict[str, Any], Any], ft.Control]] = {}


def register_field_builder(field_type: str, builder: Callable[[Dict[str, Any], Any], ft.Control]):
    FIELD_BUILDERS[field_type] = builder


def build_settings_field(field_config: Dict[str, Any], state) -> Optional[ft.Control]:
    """Create a control for a field config using registered builders.

    Each builder receives (field_config, state) and returns a ft.Control.
    """
    ftype_val: str = str(field_config.get("type") or "")
    builder = FIELD_BUILDERS.get(ftype_val)
    if builder is None:
        logger.debug(f"No builder registered for field type: {ftype_val}")
        return None
    try:
        return builder(field_config, state)
    except Exception as e:
        logger.debug(f"Field builder failed for type {ftype_val}: {e}")
        return None


# Default builders using existing modern components
def _text_builder(cfg: Dict[str, Any], state) -> ft.Control:
    label: str = str(cfg.get("label", cfg.get("key", "")))
    section: str = str(cfg.get("section", ""))
    key: str = str(cfg.get("key", ""))
    default = cfg.get("default")
    value = state.get_setting(section, key, default) if section and key else default
    validator = cfg.get("validator")
    from utils.validators import VALIDATORS  # local import to avoid cycles
    validator_fn = VALIDATORS.get(validator) if isinstance(validator, str) else None
    return create_modern_text_field(
        label=label,
        value=value,
        state=state,
        section=section,
        ...
        width=cfg.get("width"),
        keyboard_type=cfg.get("keyboard"),
        prefix_icon=cfg.get("icon"),
        suffix_text=cfg.get("suffix"),
    )


def _switch_builder(cfg: Dict[str, Any], state) -> ft.Control:
    label: str = str(cfg.get("label", cfg.get("key", "")))
    section: str = str(cfg.get("section", ""))
    key: str = str(cfg.get("key", ""))
    default = cfg.get("default")
    value = state.get_setting(section, key, default) if section and key else default
    return create_modern_switch(
        label=label,
...
def _dropdown_builder(cfg: Dict[str, Any], state) -> ft.Control:
    label: str = str(cfg.get("label", cfg.get("key", "")))
    section: str = str(cfg.get("section", ""))
    key: str = str(cfg.get("key", ""))
    default = cfg.get("default")
    ...
    for opt in raw_options:
        if isinstance(opt, ft.dropdown.Option):
            ...
            try:
                v, t = opt
                options.append(ft.dropdown.Option(v, t))
            except Exception:
                options.append(ft.dropdown.Option(str(opt)))
    return create_modern_dropdown(
        label=label,
        value=str(value) if value is not None else str(default) if default is not None else "",
        options=options,
        state=state,
        section=section,
        key=key,
...
def _slider_builder(cfg: Dict[str, Any], state) -> ft.Control:
    label: str = str(cfg.get("label", cfg.get("key", "")))
    section: str = str(cfg.get("section", ""))
    key: str = str(cfg.get("key", ""))
    default = cfg.get("default")
    value = state.get_setting(section, key, default) if section and key else default
    return create_modern_slider(
        label=label,
        value=float(value) if value is not None else float(default) if default is not None else 0.0,
        min_val=float(cfg.get("min", 0.0)),
        max_val=float(cfg.get("max", 100.0)),
        divisions=int(cfg.get("divisions", 10)),
        state=state,
        section=section,
        key=key,
...
register_field_builder("text", _text_builder)
register_field_builder("switch", _switch_builder)
register_field_builder("dropdown", _dropdown_builder)
register_field_builder("slider", _slider_builder)


# ---- Status header helper ----
def create_status_header(server_bridge, settings_state):
    """Return (container, update_fn) for the status header used in Settings view."""
    ...
    def update_fn():
        lines = container.content.controls
        if server_bridge:
            conn = "Connected to server" if server_bridge.is_connected() else "Using local storage"
            lines[0].controls[1].value = conn
        else:
            lines[0].controls[1].value = "No server bridge"
        if settings_state.last_saved:
            lines[1].controls[1].value = f"Last saved: {settings_state.last_saved.strftime('%H:%M:%S')}"
        elif settings_state.last_loaded:
            lines[1].controls[1].value = f"Last loaded: {settings_state.last_loaded.strftime('%H:%M:%S')}"
        else:
            lines[1].controls[1].value = "No recent activity"
        if getattr(container, 'page', None) is not None:
            container.update()

    return container, update_fn
...
# ========================================================================
# SETTINGS UI COMPONENTS (Extracted from views/settings.py)
# These components were moved here to reduce over-engineering in settings.py
# and follow Framework Harmony principles by centralizing UI components
# ========================================================================

def create_enhanced_field_handler(state, section: str, key: str, validator=None, data_type=str):
...
def create_enhanced_switch_handler(state, section: str, key: str, on_change_callback: Optional[Callable] = None):
...
def create_modern_reset_button(field: ft.Control, default_value: Any, state, field_type: str = "text"):
