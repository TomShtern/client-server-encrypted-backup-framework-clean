    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(sock, client, payload, *args, **kwargs):
            try:
                ...
                logger.error(
                    f"Error in request handler {func.__name__} for client '{client_name}': {e}", exc_info=True
                )
...
def handle_request_errors_detailed(
    network_server,
        ...
        def wrapper(sock, client, payload, *args, **kwargs):
            try:
                ...
                if exc_type_name == "ProtocolError" or "protocol" in str(e).lower():
                    ...
                    crypto_term in exc_type_name.lower() or crypto_term in str(e).lower()
                    for crypto_term in ["crypto", "rsa", "encryption", "decryption", "key"]
                ):
                    logger.critical(
                        f"Cryptography error in {func.__name__} for client '{client_name}': {e}",
                        exc_info=True,
                    )
...
def handle_specific_request_errors(
    network_server,
    protocol_error_code: int = 0x00000002,
    server_error_code: int = 0x00000003,
    crypto_error_code: int = 0x00000001,
    generic_error_code: int = 0x00000001,
    protocol_error_handler: Callable | None = None,
    server_error_handler: Callable | None = None,
    crypto_error_handler: Callable | None = None,
):
    """
        ...
        def wrapper(sock, client, payload, *args, **kwargs):
            try:
                ...
                if exc_type == "ProtocolError" or "protocol" in str(e).lower():
                    ...
                    crypto_term in exc_type.lower() or crypto_term in str(e).lower()
                    for crypto_term in ["crypto", "rsa", "encryption", "decryption", "key"]
                ):
                    logger.critical(
                        f"Cryptography error in {func.__name__} for client '{client_name}': {e}",
                    ...
                    logger.error(
                        f"Unexpected error in {func.__name__} for client '{client_name}': {e}", exc_info=True
                    )
                    response_code = generic_error_code
...
def handle_registration_request_errors(
    network_server,
    protocol_error_code: int = 0x00000002,
    server_error_code: int = 0x00000003,
    generic_error_code: int = 0x00000001,
...
def safe_db_operation(func: Callable, *args, **kwargs) -> Any | None:
    """
