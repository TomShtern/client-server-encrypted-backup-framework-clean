...
class ThreadManager:
    ...
    def register_thread(self, name: str, thread_obj: threading.Thread, 
                       ...
                       cleanup_callback: Optional[Callable[[], None]] = None) -> str:
        """
        ...
        with self.lock:
            ...
            setattr(thread_obj, 'stop_event', stop_event)
            setattr(thread_obj, 'thread_manager_name', name)
    ...
    def shutdown_all(self, timeout: Optional[float] = None) -> Dict[str, bool]:
        """
        ...
        with self.lock:
            ...
            for name, thread_info in self.threads.items():
                ...
                if thread_info.thread_obj.is_alive():
                    ...
                    if thread_info.state in [ThreadState.RUNNING, ThreadState.STOPPING]:
                        thread_info.state = ThreadState.STOPPED
...
def register_thread(name: str, thread_obj: threading.Thread, component: str = "unknown", 
                   daemon: bool = True, cleanup_callback: Optional[Callable[[], None]] = None) -> str:
...
def create_managed_thread(target: Callable, name: str, component: str = "unknown", 
                         daemon: bool = True, cleanup_callback: Optional[Callable[[], None]] = None,
                         args: tuple = (), kwargs: Optional[Dict[str, Any]] = None,
                         auto_start: bool = True) -> Optional[str]:
...
def shutdown_all_threads(timeout: Optional[float] = None) -> Dict[str, bool]:
