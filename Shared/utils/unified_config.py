...
class LegacyConfigurationAdapter:
    ...
    def read_port_info(self, file_path: Optional[str] = None) -> Optional[int]:
        ...
        if file_path is None:
            search_paths = [
                self.base_path / "server" / "port.info",
                self.base_path / "port.info"
            ]
        else:
            search_paths = [Path(file_path)]
    ...
    def create_migration_plan(self) -> List[ConfigurationMigration]:
        ...
        if transfer_config:
            migrations.extend([
                ConfigurationMigration("transfer.info", "client.default_server_host", transfer_config['server_host']),
                ConfigurationMigration("transfer.info", "client.default_server_port", transfer_config['server_port']),
        ...
        for config_key, files in hardcoded.items():
            if files:  # Only migrate if hardcoded values found
                if config_key == 'server_port_1256':
                    ...
                    migrations.append(
                        ConfigurationMigration(f"hardcoded_in_{len(files)}_files", "server.file_storage_dir", "received_files")
                    )
            ...
            try:
                ...
                if create_backups and migration.source_file.endswith('.info'):
                    ...
                    if source_path.exists():
                        backup_path = source_path.with_suffix(f'.info.backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
