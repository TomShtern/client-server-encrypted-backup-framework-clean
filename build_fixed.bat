@echo off
REM Fixed build script for the project
...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MT /c /I"include\client" /I"include\wrappers" /I"third_party\crypto++" /I"C:\Users\tom7s\Downloads\boost_1_88_0\boost_1_88_0" /Fo:"build\client\\" src\client\*.cpp
...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MT /c /I"include\wrappers" /I"third_party\crypto++" /Fo:"build\client\\" src\wrappers\AESWrapper.cpp src\wrappers\Base64Wrapper.cpp src\cryptopp_helpers_clean.cpp src\wrappers\RSAWrapper_stub.cpp
...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MT /DCRYPTOPP_DISABLE_ASM=1 /DCRYPTOPP_DISABLE_X86ASM=1 /DCRYPTOPP_DISABLE_X64ASM=1 /c /I"third_party\crypto++" /Fo:"build\third_party\crypto++\\" third_party\crypto++\base64.cpp third_party\crypto++\cryptlib.cpp third_party\crypto++\files.cpp third_party\crypto++\filters.cpp third_party\crypto++\hex.cpp third_party\crypto++\misc.cpp third_party\crypto++\mqueue.cpp third_party\crypto++\queue.cpp third_party\crypto++\allocate.cpp third_party\crypto++\algparam.cpp third_party\crypto++\basecode.cpp third_party\crypto++\fips140.cpp third_party\crypto++\cpu.cpp third_party\crypto++\rijndael.cpp third_party\crypto++\modes.cpp third_party\crypto++\osrng.cpp third_party\crypto++\sha.cpp third_party\crypto++\iterhash.cpp third_party\crypto++\hrtimer.cpp third_party\crypto++\rdtables.cpp third_party\crypto++\strciphr.cpp
...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MT /Fe:"client\EncryptedBackupClient.exe" build\client\*.obj build\third_party\crypto++\*.obj ws2_32.lib advapi32.lib user32.lib gdi32.lib shell32.lib crypt32.lib /link /SUBSYSTEM:CONSOLE
...
if exist "client\EncryptedBackupClient.exe" (
    ...
    echo   cd client
    echo   .\EncryptedBackupClient.exe
) else (
