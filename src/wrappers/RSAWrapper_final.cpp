...
bool RSAPrivateWrapper::loadExistingKeys() {
    try {
        // Try to load from binary DER files first (newer format)
        std::ifstream privDerFile("rsa_private.der", std::ios::binary);
        std::ifstream pubDerFile("rsa_public.der", std::ios::binary);
        
        if (privDerFile.is_open() && pubDerFile.is_open()) {
            std::cout << "[DEBUG] RSAPrivateWrapper: Loading keys from DER files..." << std::endl;
            
            // Read private key
            std::string privData((std::istreambuf_iterator<char>(privDerFile)), std::istreambuf_iterator<char>());
            std::string pubData((std::istreambuf_iterator<char>(pubDerFile)), std::istreambuf_iterator<char>());
            privDerFile.close();
            pubDerFile.close();
            
            if (!privData.empty() && !pubData.empty()) {
                try {
                    // Load private key
                    CryptoPP::ByteQueue privQueue;
                    privQueue.Put(reinterpret_cast<const CryptoPP::byte*>(privData.data()), privData.size());
                    privQueue.MessageEnd();
                    ...
                    pubQueue.Put(reinterpret_cast<const CryptoPP::byte*>(pubData.data()), pubData.size());
                    pubQueue.MessageEnd();
                    ...
                    // Store key data
                    privateKeyData.assign(privData.begin(), privData.end());
                    publicKeyData.assign(pubData.begin(), pubData.end());
                    
                    std::cout << "[DEBUG] RSAPrivateWrapper: Successfully loaded existing DER keys" << std::endl;
                    return true;
                } catch (const CryptoPP::Exception& e) {
                    std::cout << "[DEBUG] RSAPrivateWrapper: DER files exist but format incompatible: " << e.what() << std::endl;
        ...
        // Try legacy .key files
        std::ifstream legacyPriv("priv.key", std::ios::binary);
        std::ifstream legacyPub("pub.key", std::ios::binary);
        
        if (legacyPriv.is_open() && legacyPub.is_open()) {
            std::cout << "[DEBUG] RSAPrivateWrapper: Loading keys from legacy .key files..." << std::endl;
            
            std::string privData((std::istreambuf_iterator<char>(legacyPriv)), std::istreambuf_iterator<char>());
            std::string pubData((std::istreambuf_iterator<char>(legacyPub)), std::istreambuf_iterator<char>());
            legacyPriv.close();
            legacyPub.close();
            ...
            if (!privData.empty() && !pubData.empty()) {
                ...
                try {
                    ...
                    std::cout << "[DEBUG] RSAPrivateWrapper: Successfully loaded legacy keys" << std::endl;
                    return true;
                } catch (const CryptoPP::Exception& e) {
                    std::cout << "[DEBUG] RSAPrivateWrapper: Legacy key format not compatible: " << e.what() << std::endl;
                    // Clean up failed attempt
                    if (privateKeyImpl) {
                        delete static_cast<CryptoPP::RSA::PrivateKey*>(privateKeyImpl);
                        privateKeyImpl = nullptr;
        ...
        std::cout << "[DEBUG] RSAPrivateWrapper: Could not load existing keys: " << e.what() << std::endl;
    }
    
    return false;
}

void RSAPrivateWrapper::saveKeysToData() {
    if (!privateKeyImpl || !publicKeyImpl) return;
    ...
    try {
        ...
        publicKey->DEREncode(publicQueue);
        ...
        privateQueue.Get(reinterpret_cast<CryptoPP::byte*>(privateKeyData.data()), privateSize);
        publicQueue.Get(reinterpret_cast<CryptoPP::byte*>(publicKeyData.data()), publicSize);
        
        std::cout << "[DEBUG] RSAPrivateWrapper: Keys saved to internal data structures" << std::endl;
    } catch (const CryptoPP::Exception& e) {
        std::cerr << "[ERROR] RSAPrivateWrapper: Failed to save keys to data: " << e.what() << std::endl;
    }
}

void RSAPrivateWrapper::saveKeysToFiles() {
    try {
        // Save to DER files
        std::ofstream privFile("rsa_private.der", std::ios::binary);
        std::ofstream pubFile("rsa_public.der", std::ios::binary);
        
        if (privFile.is_open() && pubFile.is_open()) {
            privFile.write(reinterpret_cast<const char*>(privateKeyData.data()), privateKeyData.size());
            pubFile.write(reinterpret_cast<const char*>(publicKeyData.data()), publicKeyData.size());
            privFile.close();
            pubFile.close();
            
            std::cout << "[DEBUG] RSAPrivateWrapper: Keys saved to DER files" << std::endl;
        }
        
        // Also save to legacy format for compatibility
        std::ofstream legacyPriv("priv.key", std::ios::binary);
        std::ofstream legacyPub("pub.key", std::ios::binary);
        
        if (legacyPriv.is_open() && legacyPub.is_open()) {
            legacyPriv.write(reinterpret_cast<const char*>(privateKeyData.data()), privateKeyData.size());
            legacyPub.write(reinterpret_cast<const char*>(publicKeyData.data()), publicKeyData.size());
            legacyPriv.close();
            legacyPub.close();
            
            std::cout << "[DEBUG] RSAPrivateWrapper: Keys saved to legacy files" << std::endl;
        }
        
    } catch (const std::exception& e) {
        std::cerr << "[ERROR] RSAPrivateWrapper: Failed to save keys to files: " << e.what() << std::endl;
    }
}
