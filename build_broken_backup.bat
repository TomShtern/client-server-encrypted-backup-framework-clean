...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MD /DNDEBUG /O2 /c /I"include\client" /I"include\wrappers" /I"third_party\crypto++" /I"C:\Users\tom7s\Downloads\boost_1_88_0\boost_1_88_0" /Fo:"build\client\\" ^
...
echo Compiling production wrappers (using Crypto++ RSA implementation)...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MD /DNDEBUG /O2 /D_SILENCE_STDEXT_ARR_ITERS_DEPRECATION_WARNING /c /I"include\wrappers" /I"third_party\crypto++" /Fo:"build\client\\" ^
...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MD /DNDEBUG /O2 /c /I"third_party\crypto++" /Fo:"build\client\\" ^
...
echo Compiling Crypto++ sources with correct flags...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MD /DNDEBUG /O2 /DCRYPTOPP_DISABLE_ASM=1 /DCRYPTOPP_DISABLE_X86ASM=1 /DCRYPTOPP_DISABLE_X64ASM=1 /DCRYPTOPP_DISABLE_AESNI=1 /DCRYPTOPP_DISABLE_SHANI=1 /D_SILENCE_STDEXT_ARR_ITERS_DEPRECATION_WARNING /c /I"third_party\crypto++" /Fo:"build\third_party\crypto++\\" ^
...
third_party\crypto++\randpool_no_cfb.cpp ^
...
echo Linking production executable with Crypto++ RSA...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MD /DNDEBUG /O2 /Fe:"client\EncryptedBackupClient.exe" ^
...
ws2_32.lib advapi32.lib user32.lib gdi32.lib shell32.lib crypt32.lib msimg32.lib comdlg32.lib /link /SUBSYSTEM:CONSOLE
...
REM 1.7) Compile Debug Crypto Init Test
echo Compiling Debug Crypto Init Test...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MT /c /I"include\client" /I"include\wrappers" /I"third_party\crypto++" /Fo:"build\client\\" ^
debug_crypto_init.cpp

if %ERRORLEVEL% neq 0 (
    echo ERROR: Debug Crypto Init Test compilation failed
    exit /b 1
)

REM 1.8) Compile Simple Web Server for HTML Client
...
if %ERRORLEVEL% neq 0 (
    echo WARNING: Simple Web Server compilation failed - HTML client may not work
)

REM 5) Build Debug Crypto Init Test
echo Building Debug Crypto Init Test...
"%CL_PATH%" /EHsc /D_WIN32_WINNT=0x0601 /std:c++14 /MT /Fe:"client\debug_crypto_init.exe" ^
build\client\debug_crypto_init.obj ^
build\client\algebra_implementations.obj ^
build\third_party\crypto++\*.obj ^
ws2_32.lib advapi32.lib user32.lib gdi32.lib shell32.lib crypt32.lib msimg32.lib comdlg32.lib /link /SUBSYSTEM:CONSOLE

if %ERRORLEVEL% neq 0 (
    echo ERROR: Debug Crypto Init Test linking failed
    exit /b 1
) else (
    if exist "client\debug_crypto_init.exe" (
        echo Debug Crypto Init Test built successfully at client\debug_crypto_init.exe
    ) else (
        echo ERROR: Debug Crypto Init Test build failed
        exit /b 1
    )
)

REM 6) Build Simple Web Server for HTML Client
