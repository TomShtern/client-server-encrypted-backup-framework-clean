...
### 🔴 CRITICAL: Redundant HTML Files
**Issue:** Two nearly identical HTML files exist:
- `index.html` (713 lines) - Primary entry point
- `web_gui_content.html` (525 lines) - Legacy/duplicate
...
**Recommendation:**
1. Consolidate into single `index.html`
2. Remove `web_gui_content.html`
...
### 🔴 CRITICAL: Oversized JavaScript Files
**Issue:** Files exceed maintainability thresholds:
- `ui.js`: **1,782 lines** (recommended max: 500)
- `app.js`: 1,280 lines (acceptable but large)
- `styles.css`: **1,542 lines** (recommended max: 800)
...
### 🟡 HIGH: Class Extraction Needed
**Issue:** `TransferHistory` class lives inside `app.js` instead of separate module
...
**Issues:**
1. TransferHistory class should be extracted (lines 12-105)
2. `#showExportMenu` creates DOM on every call (line 378)
3. Long `#render` method (25+ lines)
...
**Issues:**
1. ProfessionalGUIEnhancements is 400+ lines (lines 749-1416)
2. setupShortcuts is 100+ lines (lines 1119-1239)
3. Global function pollution (updateDualServerStatus)
...
### 1. Remove web_gui_content.html (15 min)
...
|---------------------------------|---------|--------|----------------|
| **Total JS Lines**              | 4,739   | 4,500  | 🟡 -5% needed  |
| **Largest File**                | 1,782   | 500    | 🔴 -72% needed |
| **Cyclomatic Complexity (avg)** | 8.2     | 5.0    | 🟡 High        |
| **Code Duplication**            | ~8%     | <3%    | 🟡 Moderate    |
| **Test Coverage**               | 0%      | 60%+   | 🔴 None        |
| **JSDoc Coverage**              | 35%     | 80%+   | 🟡 Low         |
| **CSS File Size**               | 1,542   | 800    | 🔴 -48% needed |
| **Magic Numbers**               | 47      | 0      | 🔴 High        |
...
web_gui_content.html  525 lines   DUPLICATE - REMOVE
```
